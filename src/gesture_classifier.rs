...
impl GestureClassifier {
    pub fn new(model_path: &str, classes_path: &str) -> Result<Self, ClassifierError> {
        ...
        let prob_output_name = session.outputs
            .iter()
            .find(|output| matches!(output.output_type, ValueType::Tensor { ty: TensorElementType::Float32, .. }))
    ...
    fn load_classes(path: &str) -> Result<Vec<String>, ClassifierError> {
        ...
        let mut pairs: Vec<(usize, String)> = data.index_to_class
            .into_iter()
    ...
    pub fn predict_single(&mut self, window: &GestureWindow) -> Result<(String, f32), ClassifierError> {
    ...
    pub fn predict_scores(&mut self, window: &GestureWindow) -> Result<HashMap<String, f32>, ClassifierError> {
        ...
        let (prob_shape, prob_data) = outputs[self.prob_output_name.as_str()].try_extract_tensor::<f32>()?;
    ...
    pub fn predict_with_voting(&mut self, windows: &VotingWindows) -> Result<(String, f32), ClassifierError> {
        ...
        let (winner, _votes) = vote_counts
            .iter()
            .max_by_key(|(_, &count)| count)
            .ok_or(ClassifierError::NoOutputTensor)?;
        
        // Confidence promedio del ganador
        let avg_confidence = accumulated_scores[winner] / 5.0;
        
        Ok((winner.clone(), avg_confidence))
    }
